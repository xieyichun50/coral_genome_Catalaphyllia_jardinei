...
DEG<-read.delim("RSEM.gene.counts.matrix.R23oC_vs_R29oC.edgeR.DE_results", header = TRUE)
##Filter DEGs →
...
GenesKOGpair.1v1<-read.delim("D:/coral/eggnog/gg/KOG.1v1.txt", header = TRUE)
GenesKEGGpair.1v1<-read.delim("D:/coral/eggnog/gg/KEGG.1v1.txt", header = TRUE)
GenesGOpair.1v1<-read.delim("D:/coral/eggnog/gg/GO.1v1.txt", header = TRUE)
Geneskopair.1v1<-read.delim("D:/coral/eggnog/gg/KO.1v1.txt", header = TRUE)
...
####KEGG enrich
{
  ...
  plotdata$ratio2[plotdata$change=="Down-regulated"]<- (0-plotdata$ratio2[plotdata$change=="Down-regulated"])
  plotdata1<-subset(plotdata, abs(ratio2)>0 & p.adjust< 0.2 & ONTOLOGY != "Human\nDiseases")
  a<-plotdata1 %>%
  ...
  a
  ggsave(paste0(ref,"KEGG.tiff"), width = 10, height = 8, units = "in", dpi = 300)
  ggsave(paste0(ref,"KEGG.png"), width = 10, height = 8, units = "in", dpi = 300)
  f = paste0(ref,"KEGG.pptx")
  topptx(a,f, width = 10, height = 8, units = "in")
}
...
####ko enrich
{
  ...
  plotdata$ratio2[plotdata$change=="Down-regulated"]<- (0-plotdata$ratio2[plotdata$change=="Down-regulated"])
  plotdata1<-subset(plotdata, abs(ratio2)>0.01 & p.adjust< 0.2 )
  a<-plotdata1 %>%
    ggplot(aes(x = ratio2, y = Description, size = Count, colour = p.adjust))+
    labs(title = "", 
         size = "Gene count", colour = "Adjust P value",
...
library(edgeR)
rawcount<-read.delim("RSEM.gene.counts.matrix.R23oC_vs_R29oC.edgeR.count_matrix", header = TRUE)
TMM<-rawcount
data=as.matrix(TMM) 
